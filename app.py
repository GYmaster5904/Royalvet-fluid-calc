...
# [1] 페이지 설정 및 보안 강화 (상단 메뉴, Deploy 버튼, GitHub 아이콘 숨기기)
...
st.title("🐾 2024 AAHA 수액 & 영양 통합 계산기 (v15.0)")
st.markdown("본 도구는 **2024 AAHA 가이드라인**과 병원 **조제 실무(KCl/Ca/AA)**를 완벽히 통합한 최종 버전입니다.")
...
with e_col4:
    ...
    st.caption("P(인) 수치는 모니터링 수치입니다.")

# --- 수액 및 전해질 계산 로직 ---

# 1. 수액량 및 속도 계산
# 심장병 환자: 유지 수액량을 50%로 제한 (AAHA Section 5)
...
if k_val < 3.5 and hourly_rate > 0:
    ...
    # 선택된 수액백 용량에 맞춘 KCl 혼합 mL 계산
    needed_k_meq_for_bag = (k_safe * weight / hourly_rate) * bag_size
    needed_k_ml_for_bag = needed_k_meq_for_bag / 2
    notices.append(f"⚠️ **저칼륨혈증**: {bag_size}mL 백에 **KCl {needed_k_ml_for_bag:.1f} mL** 혼합 (속도 {k_safe} mEq/kg/hr 기준)")

# (2) iCa 보정 (10% Calcium Gluconate 가장 추천되는 고정량 적용)
if ica < 1.1 and ica > 0 and hourly_rate > 0:
    # 추천 고정 용량: Bolus 1.0 mL/kg, CRI 0.5 mL/kg/hr
    ca_bolus = weight * 1.0
    # 백 전체에 넣어야 할 총량 계산: (시간당 필요한 mL / 시간당 수액속도) * 백사이즈
    ca_ml_per_bag = (0.5 * weight / hourly_rate) * bag_size
    
    notices.append(f"🚨 **저칼슘혈증**: 응급 Bolus **{ca_bolus:.1f} mL** (20분간 서서히 IV)")
    notices.append(f"🏥 **CRI 조제**: {bag_size}mL 백에 **10% Ca Gluconate {ca_ml_per_bag:.1f} mL** 혼합 (속도 0.5 mL/kg/hr 기준)")
...
with res_col2:
    ...
    if na > 0:
        ...
        if osm > 350: st.error("🚨 HHS 고위험: 매우 느린 속도로 수액을 교정하십시오.")
...
st.caption("Reference: 2024 AAHA Guidelines & Internal Hospital Protocols (v15.0)")
